...
public class Controller extends RecordLog {
	...
	public boolean connect() {
		if (vote.LoadDriver() && vote.connect())
			return true;
		else
			return false;
	...
	public void recordLoginFail(String userName) {
		try {
			record(" Username: " + userName + " - " + getCurMessage()
					+ " |IP: "
	...
	/**
	 * call VoterDao to check that this user can summit this vote to database or
	 * not
	 * 
	 * @param user_id
	 * @param project_id
	...
	public boolean canInsert(int user_id, int project_id, int question_id,
		...
		return vote
				.canInsert(user_id, project_id, question_id, score, votetime);
	}
	...
	public DefaultTableModel voteResult(DefaultTableModel model, int id)
			throws SQLException {
